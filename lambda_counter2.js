...
function convertToDynamoDB(queryObj) {
    ...
    return {
        TableName: `db-${table_name}`, // Prefixing "db-" as in the example
        FilterExpression: `${comparison_var} ${comparisonOperators[comparison]} :val`,
        ExpressionAttributeValues: {
            ':val': { N: String(comparison_val) } // Assuming numeric comparison
        },
...
exports.lambdaHandler = async (event) => {
    ...
    // Example Input: "SELECT sub_name FROM issuers WHERE approval_status <> 0"
    const queryObj = {
        columns: ['sub_name'],
        table_name: 'issuers',
...
if (!USE_DYNAMODB) {
    ...
    app.get(`/${TRUST_ANCHOR_NAME}/subordinate_listing`, async (req, res) => {
        const response = await exports.lambdaHandler({});
